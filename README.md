...
deterministic autofix rule, the KAI grammar, tier routing, cap-fill
allocation, chat templates, the D language registry, temp-project best
resolution, two-stage scoring, budget semantics, and the LLM repair flow
(440+ tests, no network, temp dirs only).
